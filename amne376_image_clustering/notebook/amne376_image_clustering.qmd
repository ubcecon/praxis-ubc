...
**Principal Component Analysis (PCA)** is one of such techniques, it is essentially finding the two or three major axes through our huge data along which the data has the most variations. By PCA we decompose our data with 1024 dimensions (number of cells in each row) to 3 dimensions and represent each image as a point on our scatterplots. We then color the data with "era" and "material" respectively. To visualize all 3 dimensions, here we employed a strategy called **scatter matrix**. What do you notice by looking at the plots below: 
...
**t-distributed stochastic neighbor embedding (t-SNE)** is another dimensionality reduction technique. Different from PCA, instead of finding out the principal axes from the data, it assigns a certain low-dimensional space to each point based on its high-dimensional information and preserves the relative spatial relationships. You can imagine its arrangement for image embeddings as arranging the standing positions for graduation photos by surveying the students' information, trying to satisfy each student's preference (e.g., A and B want to stand together, but B and C want to be as far apart as possible).

Here, we also want to label the scatterplots based on "era" and "material", respectively, and we used `plotly` to create an interactive visualization. Interact with the points and browse the corresponding images in the folder, what pattern can you see? 
...
Although t-SNE shows a clear distinction between materials (especially between bronze and marble), we should be wary of the conclusion that the t-SNE method is somewhat randomized, so its results are not necessarily deterministic. in any case, dimensionality reduction techniques give us a good understanding of the relationship between different images.

### 6. Classification of Kouroi 

#### 6.1 Traditional Approach

In addition to observing how the labels are clustered based on the embeddings, we can train classifiers to categorize objects into appropriate labels based on the image embeddings directly. A traditional approach is through a technique called **logistic regression**. Below is a visualization showing how it works.
...
# Annotate boundary
plt.text(0.1, 0.05, 'Decision Boundary (x=0)', va='bottom')
plt.text(-4.8, 0.52, 'Threshold (y=0.5)', va='bottom')
...
In this example, we consider $x$ as the sole predictor on which the model makes its decisions, and the vertical dashed line as its decision boundary, i.e., envisioning a yes/no scenario, everything to the right of $x=0$ will be considered "yes", and everything to the left will be treated as "no". While classifiers in real life are likely much more complicated with more predictors and more labels than yes/no, they follow the same principle. Let's go back to our own example and see how it work for both labels.  
...
It reflects the proportion of true predictions out of all predictions made using the classifier. However, as shown above in the report, the accuracy of predicting era based on image embedding is not satisfactory.

We want to see what is exactly going wrong, so we display all the prediction results against true values by generating a **confusion matrix**. Note that true predictions are on the main diagonal of this matrix.
...
Can you find any noteworthy pattern from it? Do you think the image embeddings captured enough characteristics to classify eras of the Kouroi?
...
Again, let's generate a confusion matrix and see how exactly does the prediction results distribute. What can you see? Would you consider it as a good result? 
```{python}
# Plot the confusion matrix
...
The above results show that the reason for the low precision is that other materials that are not marble or bronze are classified as marble or bronze. however, since the dataset itself has a small number of Kouroi samples for other materials, we cannot conclude that using logistic regression to classify materials is not a good idea.
...
#### 6.3 Risk of Underfitting and Overfitting
...
- **Image embeddings** produced by a pretrained CNN map each kouros image into a high‑dimensional feature space. We can then apply **dimensionality‑reduction techniques** like **Principal Component Analysis** or **t‑distributed Stochastic Neighbor Embedding (t‑SNE)** to visualize clusters among different archaic sculptures.
...
- **t-distributed Stochastic Neighbor Embedding (t-SNE)**: t-SNE is a powerful technique for visualizing high-dimensional data in lower dimensions, typically two or three, while preserving the local structure of the data. It does this by converting similarities between data points in the high-dimensional space into probabilities and then minimizing the difference between these probabilities and those of the low-dimensional representation. 
...
### Appendix: Image Data Collection and Preprocessing from `.pdf` Files
