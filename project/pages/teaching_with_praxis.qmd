...
Quarto is a strong improvement over R Markdown and supports multiple languages.  However, it is still in development and is more complicated.  **We recommend Quarto for development and offline use**^[We use Quarto to develop the this project.].
...
Most flipped classroom experiences tend to use videos (see @akccayir2018flipped), and this is quite feasible with Jupyter Notebooks.  Record yourself demonstrating the notebook, and have students follow along.
...
* For example, you could assign students a pre-reading of a notebook before class.  In the class, you could then introduce a new dataset and have them explore it using clustering methods.
* Many of our notebooks are built with this structure, where one "Part" of the notebook can be turned into the classroom part of the flipped classroom.^[This was actually our original use for the notebooks, before COMET! See @graves2024understanding]
...
* Many of our notebooks are built with this structure.
...
While we think that teaching using a JupyterHub is the best option, that may not always be desired or possible.  If you want to use our notebooks, or similar teaching tools, without a JupyterHub you have two main options:
