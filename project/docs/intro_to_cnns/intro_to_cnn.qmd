...
::: {.callout-warning}
This notebook may require using a GPU and a system with over 24GB of RAM to run efficiently. Running it on a system with limited resources may result in long processing times or potential memory issues.

Note that some code cells are commented out to prevent long execution times. You can choose to uncomment and run them if your system has sufficient resources.
:::
...
```{python}
# !pip install opencv-python numpy matplotlib pandas scikit-learn seaborn datasets torch tqdm
```

![A visual explanation of CNN](media/CNN_example.jpg){width="800"}

### 1. What are Neural Networks?

What is the first thing that comes to your mind when you hear the word "**neural network**"? If you are thinking about the human brain and neurons, you are not wrong. In fact, the term "neural network" is inspired by the way the human brain and nervous system work, where neurons are connected to each other and communicate with each other to process information. 
...
Now that we understand what is an artificial neural network, let's dive into the real topic here: What's unique about **convolutional neural networks (CNNs)** and why they are revolutionary to computer vision and image processing? 
...
Can you see what the image is about? Can you imagine how a computer understands it?
...
Architecture determines the way how a CNN integrate and learn from the features it extracted, and thus largely determines the performance of a model. Throughout the years, there have been several hugely successful CNN architectures, which we won't be able to discuss in detail. Here, I will only demonstrate the architecture of **LeNet-5**: It reads images in a sequence that starts with a partial and combines the partials into a comprehensive one. Intuitively, the learning process of this architecture can be thought of as learning to write a new character: You learn to write each stroke first, and then follow the structure of the character to put those strokes together into a complete character.

![A visual comparison of two neural networks](media/lenet5.png){width="700"}
...
Next, we will set the loss and optimizer function used during the training. These are the factors that determine how much your network learns from the mistakes and adjust the distribution of weights for how it trusts the "experts". 
...
We can visualize the feature maps at different layers to see how the CNN sees images.
...
We can see that the model made mostly correct predictions, despite the image quality being so low that even a human may have difficulty to correctly classify. This somewhat shows the advantage of CNNs over humans when confronted with complex, blurry images, but CNNs have more applications than that. They power a host of real-world applications, from enabling your smartphone’s camera to automatically recognize faces and apply portrait effects, to guiding autonomous vehicles by detecting pedestrians, road signs, and lane markings in real time. In healthcare, CNNs help radiologists spot tumors in MRI and CT scans, and dermatologists classify skin lesions from photos. They underpin optical character recognition for digitizing handwritten forms, fuel quality-control systems that spot manufacturing defects on assembly lines, and even drive wildlife monitoring by identifying animals in camera-trap images.
...
3. **Architectures** are central in neural networks as they determine the ways a model learns from the input features and thereby determine the model performance.
