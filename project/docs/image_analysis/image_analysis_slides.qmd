...
:::{layout-ncol=2}
![Convolution Operation](media/sharpener_kernel.gif){height=120px fig-align="center"}

![Feature Extraction](media/eye_detection_kernel.gif){height=120px fig-align="center"}
:::
...
![Sobel filter example (Credit: Praxis UBC)](media/sobel_kernel_example.png){height=200px fig-align="center"}

A Sobel filter highlights edges by computing intensity gradients in the horizontal and vertical directions.
...
Start with the simplest pre-trained model that fits your task and compute budget.
...
The examples in this workshop are **designed to teach concepts, not to achieve production-quality results**.

Some outputs (for example OCR on archival photos) will look rough. That is intentional: the goal is to unpack the black box of image AI step by step, so you understand how each tool works and where it breaks down.

. . .

For real projects, you would use larger models, cleaner data, and careful tuning. What matters here is the intuition.
...
:::{layout-ncol=2}
![ResNet-18](media/Original-ResNet-18-Architecture.png){width=160px}

![ViT](media/ViT_architecture.png){width=160px}
:::
...
Our demo example deliberately uses a challenging archival image: the OCR output is imperfect, which helps illustrate what to expect before you invest time tuning.
...
![Captioning architecture](media/image_captioning_transformer.png){height=180px fig-align="center"}

. . .

**The gap:** The model sees only pixels -- it doesn't know *who*, *where*, or *why*.

---

## Enriching Captions with Context

Raw captions describe surfaces ("aerial view of urban landscape") but miss context ("Cleveland on Lake Erie, 1973").
