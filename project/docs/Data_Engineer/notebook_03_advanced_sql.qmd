...
1. Nest a query inside another query as a **derived table**, name query steps with **CTEs** (`WITH`), save whole queries as **views**, and choose between the three by how long the query needs to live.
...
3. Modify data with **INSERT**, **UPDATE**, and **DELETE**, and protect yourself from the update that forgets its `WHERE`.
...
The dataframe from Notebook 2 was sent to your co-workers, and word about your skills got around. You are now the research team's SQL person, and being the SQL person means having a queue. This morning it holds six tickets: an old debt from the research lead, a scheduling request from the whole team, two analysis asks, three admin items from the survey office, and a teammate's code waiting for review. Every single one needs a tool we do not have yet, and together those tools are the second half of SQL, the half that separates "knows a bit of SQL" from "uses it daily at work".

## 2. Setup

We rebuild the database from the raw files, so this notebook can be run alone (without needing to run NB1 or NB2):
...
Both cells print nothing, and that is what we are expecting: `CREATE VIEW` stores the query and returns no rows. That stored query is Notebook 2's entire deliverable, and re-running the briefing is now a one-liner, for every teammate who opens this database file:
...
One thing to hold on to: a view stores the **query**, but not the results. Every `SELECT` against it re-runs the briefing on whatever the data looks like at that moment, which is exactly what the team wants on Fridays.

::: {.callout-important}
## The naming ladder

A **subquery** is anonymous work inside a single query. A **CTE** names a step for the duration of one query, for readability and stacking. A **view** names a whole query forever, inside the database file, for everyone. Choose by how long the query needs to live.
:::

## 5. Tickets three and four: window functions

> **Ticket #3, from the comms team.** "Top three earners in each province, for the salary profile piece."

This sounds routine until you try it with Notebook 2's tools. `GROUP BY province` collapses each province to one row, so the people dissapear just when we want to point at them. `ORDER BY hourly_wage DESC LIMIT 3` keeps people but ranks the whole country, three rows total. This exact gap is what **window functions** exist to fill: they compute group-level facts, like a rank within a province, while **keeping every row**.
...
SQLite refuses: `misuse of aliased window function`. You already have seen a refusal shaped exactly like this before, and it is Notebook 2's evaluation order: `WHERE` runs early, before the `SELECT` clause where the window is computed, so at filtering time `province_rank` does not exist. To fix this we use sections 3's tool. We compute the window inside a CTE, then filter the CTE's output, where the rank is a regular column:
...
Eighteen rows, three per province, every earner still exists as an individual row. Ticket three ships to the comms team.
...
Two details inside. The `* 1.0` exists because `position` and the row count are whole numbers, and SQLite keeps whole-number division whole; multiplying by 1.0 first forces the decimal shares we want. And the two queries in parentheses are section 3's idea again, subqueries quietly supplying the totals to divide by.
...
The bottom half of earners take home about 37% of the wages. If the curve hugged the dashed diagonal, every worker would earn the same; the deeper it sags, the more unequal the distribution. Chart attached as an email reply, ticket four closed. 
...
One distinct value in the whole column: every one of a thousand respondents now works exactly 40 hours a week. What happened, and what is the habit that prevents it?
...
Now the serious one: respondent 731 has withdrawn consent. We have to adress this, participation is voluntary, and the deletion must happen. Look first, as always, then delete, then verify:
...
Our attack only read data. A payload with a more vicious ending deletes it instead, which is the joke behind one of the most famous comic in programming:
...
We send the tool goes back to your teammate with two changed lines, and we are done with our ticket queue and can take a well deserved break!

## 8. The full cheatsheet

Everything in this notebook translates to Python `pandas` you already know: a CTE is a named intermediate dataframe, `RANK() OVER (PARTITION BY ...)` is `groupby().rank()`, a running total is `cumsum`, and `INSERT`, `UPDATE`, and `DELETE` are `concat`, `.loc` assignment, and boolean filtering. Both parts of the translation table, are the promised complete reference: [download the full SQL to pandas cheatsheet (PDF)](media/NB3/sql_pandas_cheatsheet.pdf){download="sql_pandas_cheatsheet.pdf"}.

## 9. Conclusion

Let's look back at what the ticket queue taught you. A subquery paid Notebook 2's debt (the certified average is 39.44, an 85-cent premium), a CTE made it readable, and a view turned the briefing into permanent team infrastructure. Window functions answered the questions `GROUP BY` cannot, ranks and running totals, and drew a Lorenz curve on the way. INSERT, UPDATE, and DELETE made the database a living object, with one near-catastrophe on the practice copy to show why the `WHERE` clause is sacred. And one small habit, `?` plus `params`, fixed SQL injections.

You are past the core of SQL now, and into the part that employers specifically probe for. The practice sites from Notebook 2 grade this material as their medium tier; on DataLemur, the tags "Window Functions" and "CTEs or Subquery" are yours to practice on.
