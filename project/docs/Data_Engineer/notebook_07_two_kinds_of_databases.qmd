...
- Extract copies data out of source systems, transform reshapes it (joins, derived columns, cleaning, conforming types), load writes it into the warehouse. The transform step creates every derivation that analysts would otherwise have to re-make everytime: flags, calendar attributes, standardized categories.
