...
5. Spot and fix plausible-looking wrong queries, and translate between SQL and `pandas`.
...
The `None` row is Saskatchewan, and it has the **highest** average wage in the table. Our three-region draft omitted 46 individuals which were the highest paid group in the survey and skewed every comparison in the process. Nothing crashed, no error warned us, and the wrong table even looked cleaner than the right one.
...
Notebook 3 adds part two (subqueries, window functions, the writing commands) and bundles both parts into one downloadable .pdf cheatsheet.
