...
3. Define **database**, **database management system (DBMS)**, and **SQL**, and state the advantages of DBMS over regular files.
...
```{mermaid}
flowchart LR
    A["Operational systems<br/>checkouts, payroll,<br/>survey software"] --> B["Databases<br/>rules enforced,<br/>nothing lost"] --> C["Extraction and cleaning<br/>queries, coding,<br/>weighting"] --> D["The CSV<br/>you are handed"] --> E["Notebooks, models,<br/>dashboards, further analysis"]
```
...
None of these ever produces an error message. A file loads correctly with three spellings of one person as with one, and the damage surfaces only later, in analysis results, where it is expensive to trace back, or even worse, they never surfaces at all. Notebook 4 hands you a survey file with every one of these problems in it and teaches you to repair it.
...
1. **Structure.** Every table declares its columns and their types up front. A wage column holds numbers, so `refused` or `none` cannot be stored in it, and a change in what a column means has to become an explict change in the structure of the database.
2. **Integrity rules.** The database can enforce that every respondent has exactly one row, that a region must come from a fixed list (for example American States), or that a wage must fall in a plausible range.
...
The kind of database we use in this stream is a **relational** database, where data lives in tables: each row is an individuals records, each column is one property of the individual (Wage, Age, Location etc), and every row has the same columns.
...
Loading it into SQLite takes two lines: connect, then write the dataframe in as a table. The connection creates `wage_survey.db` in the datasets folder, and that single file is the entire database. We also print the number of rows.
...
For the remainder of the stream we will not use pandas and `pd.read_sql`, Jupyter notebooks have a better way: the `jupysql` extension adds a `%%sql` marker that turns an entire cell into SQL. We set it up once, pointing it at our database file and asking for results as dataframes:
