...
- **Notebook 10, Databases for AI.** JSON in SQL, embeddings as coordinates of meaning, similarity search as an `ORDER BY distance`, where semantic search beats exact matching and where it loses, and RAG demystified as a database pattern.
- **Notebook 11, Capstone.** Pick a public data source and ship the full stack yourself, raw data to schema to warehouse to validated pipeline, judged on one question: can a stranger clone it, run one command, and get your result?
