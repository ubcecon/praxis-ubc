...
Note: Saskatchewan made the province list this time, since we are past those issues, so we won't run into any new NULL-related problems.
...
Question two from the survey office: respondent lookup has to stay instant. Here is the lookup, which we time like before. Respondent 54001 is the busiest person in our simulation, with 14 interviews across the two years:
...
A couple of hundred milliseconds. The number that matters today is the time. Two hundred milliseconds for one question is fine, until it is a dashboard running fifty questions on a table a hundred times bigger.
