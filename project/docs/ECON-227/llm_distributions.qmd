...
def process_sentiment(news_df):
    ...
    grouped["7day_sum_positive"] = grouped["POSITIVE"].rolling(window=7, min_periods=1).sum()
    # Calculate a rolling 7-day total of negative headlines
    grouped["7day_sum_negative"] = grouped["NEGATIVE"].rolling(window=7, min_periods=1).sum()
    # Calculate the percentage of positive headlines each day (out of total positive + negative)
    grouped["daily_pct_positive"] = grouped["POSITIVE"] / (grouped["POSITIVE"] + grouped["NEGATIVE"])
...
Let's take a closer look at the news titles.

> *What would you say about their sentiment?*
...
Now let's apply the model to our entire `SBUX_news_df` and see how each news headline is classified. Compare the sentiment labels with what you would have guessed. Do you think the model is doing a good job?
...
Let's summarize the sentiment results by date. For each day we count the number of positive and negative headlines, then calculate 7-day rolling sums and the daily percentage of positive news. This gives us a quick overview of news sentiment trends over time.
...
- If the `daily_pct_positive` is rising, the overall tone of news is getting more optimistic.
...
#### From Language Prediction to Stock Prediction
...
In the previous section, we used a language model to analyze real-time news headlines and extract sentiment signals.

Now, we will combine these sentiment signals with historical stock price data to forecast short-term changes in stock value.

You will:

-   Combine sentiment signals with stock prices
-   Use a forecasting model to predict future price changes
-   Visualize your results interactively

#### Retrieving and Preparing Stock Price Data
...
def combine_data(sent_df, stock_df):
    ...
    merged = pd.merge(sent_df, stock_df, on="DateOnly", how="inner")
    merged["lagged_sentiment"] = merged["daily_pct_positive"].shift(1)
...
def AI_process_sentiment(news_df):
    ...
    grouped["7day_sum_positive"] = (
        grouped.groupby("Ticker")["POSITIVE"]
        .transform(lambda x: x.rolling(window=7, min_periods=1).sum())
    )
    grouped["7day_sum_negative"] = (
        grouped.groupby("Ticker")["NEGATIVE"]
        .transform(lambda x: x.rolling(window=7, min_periods=1).sum())
    )

    # Compute % positive
    grouped["7day_pct_positive"] = grouped["7day_sum_positive"] / (
        grouped["7day_sum_positive"] + grouped["7day_sum_negative"]
    )
...
def fetch_actual_pct_changes(ticker_list, start_date, end_date):
    start_fetch = (start_date - pd.Timedelta(days=7)).strftime('%Y-%m-%d') # gives a string of the date 7 days before the start date
    end_fetch   = (end_date   + pd.Timedelta(days=1)).strftime('%Y-%m-%d') #gives a string, 1 days after end date.

    frames = []
    for t in ticker_list:
        df = yf.download(t, start=start_fetch, end=end_fetch, progress=False, auto_adjust=False) #we 
        if df.empty: 
            continue
        df.columns = df.columns.get_level_values(0) #we collapse the result to a standard pd df

        df.index = pd.to_datetime(df.index).normalize()
        pct = df['Adj Close'].pct_change() * 100.0
        pct = pct.loc[(df.index >= start_date) & (df.index <= end_date)].dropna()

        frames.append(pd.DataFrame({
            'DateOnly': pct.index,
            'Ticker': t,
            'Actual_Pct_Change': pct.values
        }))

    return pd.concat(frames, ignore_index=True) if frames else pd.DataFrame(
        columns=['DateOnly', 'Ticker', 'Actual_Pct_Change']
    )
...
def compare_predictions(pred_mean, pred_index, actuals_df, ticker=None):
    ...
    # Average actuals across tickers per day 
    mdf = (
        merged.groupby('DateOnly', as_index=False)
        .agg(Predicted_Pct_Change=('Predicted_Pct_Change', 'first'),
                    Actual_Pct_Change=('Actual_Pct_Change', 'mean')))
    ...
    return mdf, {'count': len(mdf), 'MAE': mae, 'RMSE': rmse}
```
...
actuals_df = fetch_actual_pct_changes(ai_tickers, compare_start, compare_end)
