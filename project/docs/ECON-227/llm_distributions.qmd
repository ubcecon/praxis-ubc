...
# Katherine suggested edits
# for a singke module 
# llm_forecasting/pipeline.py

class SentimentForecaster:
    """
    High-level forecasting pipeline that mirrors LLM-style prediction:
    context → distribution → forecast
    """

    def __init__(self, ticker, sentiment_model):
        self.ticker = ticker
        self.sentiment_model = sentiment_model

    def load_data(self, news_df, price_df):
        self.news_df = news_df
        self.price_df = price_df

    def build_features(self):
        from .sentiment import apply_sentiment, process_sentiment
        from .data import combine_data

        sentiment = apply_sentiment(self.news_df)
        sentiment_summary = process_sentiment(sentiment)
        self.dataset = combine_data(sentiment_summary, self.price_df)

    def fit(self):
        from .models import fit_sarimax
        self.model = fit_sarimax(self.dataset)
    ...
    def plot(self):
        ...
        plot_forecast(self.dataset, self.model)
...
vocab = ["buy", "sell", "hold", "wait"]
base_probs = np.array([0.5, 0.2, 0.2, 0.1])

def sample_next_word(temp):
...
widgets.interact(
    sample_next_word,
    temp=widgets.FloatSlider(min=0.1, max=2.0, step=0.1, value=1.0)
)

# next-token prediction
def next_step_prediction(context_length):
    ...
    plt.axvline(mean, color="red", label="Most likely next value")
    plt.title("Predicting the Next Token (Number)")
    plt.legend()
    plt.show()

widgets.interact(
    next_step_prediction,
    context_length=widgets.IntSlider(min=5, max=40, step=1, value=20)
)

#sentiment to map to transformers with extra cibtext embeddings
def sentiment_conditioned_prediction(sentiment_strength):
    base_return = np.random.normal(0, 1, 100)

    adjusted = base_return + sentiment_strength * 0.5

    plt.hist(adjusted, bins=30, alpha=0.7)
    plt.title(f"Sentiment-conditioned forecast\nSentiment = {sentiment_strength}")
    plt.show()

widgets.interact(
    sentiment_conditioned_prediction,
    sentiment_strength=widgets.FloatSlider(min=-2, max=2, step=0.1, value=0)
)
