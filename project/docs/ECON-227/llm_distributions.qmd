...
author: |
  prAxIs UBC Team <br>
  Krishaant Pathmananathan, Yash Mali, Kaiyan Zhang
format: html
jupyter: python3
...
![](project/docs/ECON-227/media/next_word_prediction.png)
...
![](project/docs/ECON-227/media/sequence_words.png)
...
# SBUX_news_df.to_csv("project/docs/Econ-227/datasets/SBUX_news.csv", index=False)
```
...
```{python}
SBUX_news_df = pd.read_csv("project/docs/Econ-227/datasets/SBUX_news.csv")
SBUX_news_df.head()
```
...
We are using the `yfinance` package to get real stock price data directly from Yahoo Finance. The function below helps us download historical stock prices and compute the daily percentage change in the stock's closing price. To make sure our results are replicable, this data has been saved as a `.csv` file "project/docs/Econ-227/datasets/SBUX_price.csv"
...
# stock_df.to_csv("project/docs/Econ-227/datasets/SBUX_price.csv", index=False)

SBUX_price_df = pd.read_csv("project/docs/Econ-227/datasets/SBUX_price.csv")
SBUX_price_df.head()
```
...
# AI_combined_news_df.to_csv("project/docs/Econ-227/datasets/AI_news_snapshot.csv", index=False)
# AI_sentiment_news_df = apply_sentiment(AI_combined_news_df)         
# AI_sentiment_news_df.to_csv("project/docs/Econ-227/datasets/AI_news_sentiment.csv", index=False)
```

```{python}
AI_sentiment_news_df = pd.read_csv("project/docs/Econ-227/datasets/AI_news_sentiment.csv")
...
# AI_combined_price_df.to_csv("project/docs/Econ-227/datasets/AI_top10_price_snapshot.csv", index=False)

AI_stock_prices = pd.read_csv("project/docs/Econ-227/datasets/AI_top10_price_snapshot.csv")
