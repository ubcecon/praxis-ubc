...
In part 4, we've validated our classifier on expert-labelled data. So far it's okay, but not ideal. Think of ways we could improve the performance of the model. Would using a model trained on other types of data help? 

Now, let's apply it to something noisier and more informal: financial social media posts. We'll use the twitter financial news sentiment dataset from huggingface, which contains over 21 thousand finance-related posts covering a wide range of companies and market events. 

This lets us:

1) Test the model on a completely different style of text (short, informal posts)
...
The following cell will run the model on the dataset and predict the sentiment of each sentence. Note that it may take a very long time to run so we've pre-ran the code and saved just the sentiment in a seperate csv. 
...
#     "datasets/twitter_topic_labels.csv", index=False)
```

```{python}
ticker_posts = twitter_df[twitter_df["text"].str.contains(r'\$[A-Z]{1,5}\b', regex=True)].copy()
ticker_posts["orig_index"] = ticker_posts.index
labels_df = pd.read_csv("datasets/twitter_topic_labels.csv")
...
Finance and crypto posts frequently use the `$TICKER` tag to mention stocks or coins. Let's extract those mentions and see which companies make up most of the conversation, and if the conversation about them skews positive vs negative. We previously filtered our dataset to only contain examples with said stock tickers.
...
Naturally, an important question we'd want to ask is if positive social media coverage correlates with better stock performance. For this, we'll pull 3-month stock returns for our tickers using the yfinance package and compare them to their sentiment. 
...
if os.path.exists("datasets/returns_3m.csv"): # this sucks to read, but we're just downloading the data from yfinance and trying to avoid rate limits by caching the data the second time this cell is ran
...
The scatterplot above shows a *correlation* between sentiment and returns. But correlation is not prediction. Can we actually use sentiment, combined with a company's past performance, to forcast what will happen next?
...
This kind of mirrors the core idea from section 1. The model looks at what is knows (sentiment and recent momentum of each stock ticker) and assigns weights to produce a prediction. The "vocabulary" here is just two features instead of thousands of words. 

> FYI: the stock returns are being pulled using today's date, but our sentiment data is from 2019, so the results are a bit wonky. 
