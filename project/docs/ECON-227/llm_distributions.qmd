...
SBUX_news_df.head(10)[["DateOnly", "Title"]]

# --- 3. Guess and Classify Sentiment ---
SBUX_sentiment_df = apply_sentiment(SBUX_news_df)
SBUX_sentiment_df.head()

SBUX_sentiment_df["7day_pct_positive"] = (
    SBUX_sentiment_df.groupby("DateOnly")["POSITIVE"]
    .transform(lambda x: x.rolling(7, min_periods=1).sum() /
                         (x.rolling(7, min_periods=1).sum() + x.rolling(7, min_periods=1).sum()))
)
SBUX_sentiment_df.head()

# --- 5. Forecasting Stock Changes Using Sentiment + Past Returns ---
def prepare_and_forecast(sent_df, ticker="SBUX", forecast_days=3):
    stock_df = pd.read_csv(f"datasets/{ticker}_price.csv")
    stock_df["DateOnly"] = pd.to_datetime(stock_df["DateOnly"])
    stock_df["Pct_Change"] = stock_df["Close"].pct_change() * 100

    combined_df = pd.merge(
        sent_df, stock_df, on="DateOnly", how="inner"
    ).assign(
        lagged_sentiment=lambda df: df["7day_pct_positive"].shift(1)
    ).dropna()
    ...
    future_dates = get_future_dates(combined_df.index[-1], forecast_days)
    future_exog = np.tile(combined_df['lagged_sentiment'].iloc[-1], forecast_days).reshape(-1, 1)

    forecast = fit.get_forecast(steps=forecast_days, exog=future_exog)
    return combined_df, forecast.predicted_mean, forecast.conf_int(), future_dates

combined_df, forecast_mean, forecast_ci, forecast_index = prepare_and_forecast(SBUX_sentiment_df)
create_plot(combined_df, forecast_mean, forecast_ci, forecast_index)

# --- 6. AI Stocks Sentiment Analysis ---
ai_tickers = ["ACN", "ADBE", "AMD", "APP", "AVGO", "CRM", "MRVL", "MU", "NVDA", "QCOM"]
...
# --- 7. Model Evaluation ---
compare_start, compare_end = get_compare_window(forecast_index[0], forecast_index[-1])
actuals_df = fetch_actual_pct_changes(ai_tickers, compare_start, compare_end, buffer_days=7)
pred_mean_values = forecast_mean.values
mdf, metrics = compare_predictions(pred_mean_values, forecast_index, actuals_df)
