...
ML is the backbone of modern data analysis, enabling researchers and organizations to unlock the full potential of structured data, which is cleanly and unifromly formatted, and unstructured data which may have a more complex format.
...
1. **Training**: The model first analyzes this pre-labeled data to learn the features (ex: linguistic patterns, structures, semantic relationships) associated with each category. 
...
- The "best" boundary is the one that **maximizes the distance** between the classes.
...
- It is less resource intensive: Only support vectors are stored.
...
- **$k$-means Clustering** is the simplest form of clustering: 
  ...
  - It randomly creates *k* cluster centers, then iteratively changes them to minimize the distance between them. 
