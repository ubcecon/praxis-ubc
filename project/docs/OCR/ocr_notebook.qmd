...
#!pip install pytesseract easyocr opencv-python Pillow matplotlib pandas numpy nltk jiwer pdf2image surya-ocr poppler
```
...
del rec_predictor, det_predictor
gc.collect()
``` 
...
Now that we have the handwritten letter loaded we can compare EasyOCR vs Surya OCR on the same image.
...
```{python}
rec_predictor_hw = RecognitionPredictor()
...
del rec_predictor_hw, det_predictor_hw
gc.collect()
```
...
Both engines struggle with handwritten text. Traditional OCR is built for printed characters, so the low confidence scores and garbled output here are expected. For handwritten documents, specialized tools like Transkribus or fine-tuned VLMs tend to perform much better.
...
Deep learning models like EasyOCR and Surya OCR can leverage GPUs for faster inference. Unlike CPU parallelization where you run multiple processes, GPU parallelization typically involves **batching**—feeding multiple images through the model simultaneously.
