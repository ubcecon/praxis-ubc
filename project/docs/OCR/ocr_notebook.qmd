---
title: "Optical Character Recognition (OCR) for Humanities Research"
author: "Alex Ronczewski"
date: 2026-02-03
description: A practical guide to extracting text from historical documents using Python.
categories:
  - Python
  - OCR
  - text extraction
...
2. **Extract text** from historical documents using pyTesseract and EasyOCR
3. **Pre-process images** to improve OCR accuracy
4. **Handle complex layouts** like multi-column newspapers
5. **Evaluate OCR quality** using standard metrics (CER, WER)
...
**EasyOCR** is a newer, deep-learning-based OCR library that supports 80+ languages. It often performs better on challenging images and provides built-in support for detecting text regions.
...
# Create a DataFrame mimicking Tesseract's output format
ocr_df = pd.DataFrame({
...
Traditional OCR processes text in isolation—each character or word is recognized independently. VLMs take a fundamentally different approach:

| Aspect | Traditional OCR | Vision Language Models |
|--------|-----------------|------------------------|
| **Processing unit** | Individual characters/words | Entire document or region |
| **Context awareness** | Limited to nearby characters | Full document context |
| **Layout understanding** | Requires separate analysis | Inherently integrated |
| **Ambiguity resolution** | Based on character shapes | Based on meaning and context |

For example, if a historical document has a smudged word, traditional OCR might output nonsense characters. A VLM can use surrounding context—the topic of the paragraph, common phrases of the era, grammatical structure in order to infer the most likely word.

**The Trade-off**: VLMs are more powerful but also more resource-intensive. They excel at understanding *what a document means* but may sometimes "hallucinate" plausible-sounding text that wasn't actually in the image. This will be familar to anyone who has used ChatGPT or other LLM's in the past. Traditional OCR is more literal. It only outputs what it detects, errors and all.
...
Binarization converts the image to pure black and white. **Otsu's method** **EXPLAIN HERE WHAT IT DOES** automatically finds the optimal threshold:
...
Explain what tokenization is here: 
...
#### LLM-Based Correction (Concept) - FINISH THIS SECTION
...
- Be explicit about preserving historical language
- Process in chunks to avoid context length limits
- Use examples of common OCR errors in your prompt
- Consider using Claude or GPT-4 for best results
:::
