...
Here we define helper functions for loading and displaying images, and prepare the output directory.
...
ocr_df = pd.DataFrame({
    ...
    'conf': [conf * 100 for (bbox, text, conf) in results] # convert 0-1 to 0-100
})
...
#
## PaddleOCR needs .png, so convert from .jp2 first
...
letter_pages = convert_from_path(
    "data/simple/handwritten_letter_page1.pdf",
    dpi=300,
...
```{python}
noisy_path = "data/simple/newspaper_noisy.jp2"
...
def preprocess_document(image):
    """Grayscale -> CLAHE -> denoise -> deskew -> binarize."""
    results = {}
...
And then we compare it with our OCR output to see how the model performed.
```{python}
