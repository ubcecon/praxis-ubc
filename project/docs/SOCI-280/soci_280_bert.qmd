...
**BERT:** to enable these analyses we will be using two BERT models. BERT is an open-source framework for machine learning, which is used for NLP. BERT is well-suited to understanding language, rather than generating it, which is what this notebook is concerned with. The specific BERT models we are using are a multilingual [model](https://huggingface.co/sentence-transformers/paraphrase-multilingual-MiniLM-L12-v2), which can analyze tweets in different languages, and a [model](https://huggingface.co/cardiffnlp/twitter-roberta-base-sentiment) trained for analyzing tweet sentiments.
...
```{python}
# !pip install pandas numpy torch scikit-learn matplotlib seaborn datar datasets transformers nltk ipywidgets ipython scipy kaggle detoxify tf-keras
```
...
Note that our data needs to be pulled off of the internet for us to use first. For this, we will need two separate API keys, one for Kaggle, and one for GitHub. Let' first get the Kaggle API key.
 
#### 1) [Kaggle API Setup](https://www.kaggle.com/docs/api)
 
1. Go to [https://www.kaggle.com](https://www.kaggle.com) and log in.
2. Click on your profile picture (top right) → **"Account"** and click settings.
3. Scroll down to the **"API"** section → click **"Create New API Token"**.
4. This will download a file called `kaggle.json`
 
####  Next Steps:
1. In your Jupyter environment (e.g. JupyterHub):
2. Click on the **file browser panel**.
3. Press **new folder** and create a new folder called `kaggle`.
4. Navigate to the `kaggle` folder inside your project directory.
5. **Upload** the `kaggle.json` file (you downloaded it from Kaggle earlier).
6. Your folder structure should now look like this:
...
> Once this is done, the Kaggle API will work normally in your script.
...
print("Kaggle credentials set from:", kaggle_json.resolve())
```

```{python}
from data.data_pulling import create_disinformation_dataset
```

Once that's done, run the code below to pull and generate our dataset:

```{python}
create_disinformation_dataset() #creates our dataset
```

```{python}
tweets = pd.read_csv('data/combined_disinformation_dataset_final.csv') #loads in the dataset for us to use
tweets = tweets[['language', 'date', 'time', 'source', 'is_propaganda', 'content']]
print(tweets.columns) # the columns we chose to keep for our analysis
```
...
- Is there a meaningful difference in who posts when? Why might that be?
...
1. Why are disinformation accounts posting early in the morning and late at night? What might they be trying to achieve with such an aggressive posting schedule.
...
Now that we have gone through the dataset and examined a variety of its features, take a few minutes and discuss the following questions with a partner or small group. 
...
## 1. Propaganda Tweet Classification with a Fine-Tuned Model
...
1. “#qanon #trump Hunter Biden is a Ukrainian Shill” 
...
Let’s break down this output. There are two parts to what the model returns:

1. **Label** → a classification labelling the text as either having positive, negative, or neutral sentiment 
2. **Score** → the model’s confidence in its classification
...
![Fuck dude, nurses are the shit (Mauboussin, 2022)](media/toxicity_noscore_reddit_example.png "Text: Fuck dude, nurses are the shit")
...
![Fuck dude, nurses are the shit with Toxicity Score (98.62%) (Mauboussin, 2022)](media/toxicity_reddit_example.png "Fuck dude, nurses are the shit with Toxicity Score (98.62%)")
...
![Comparing the Toxicity Scoring Models (Resende et al., 2024)](media/toxicity_aave_score_table.png "Comparing the Toxicity Scoring Models")
