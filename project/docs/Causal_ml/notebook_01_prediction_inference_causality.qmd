...
These look almost the same. They are not. The first asks for a good *guess* of the outcome. The second asks for a *cause-and-effect* number: what would happen to **the same household's** savings if we reached in and made it eligible for a 401(k), holding everything else fixed?
...
We will use the **401(k)** dataset, a standard dataset in the study of savings behaviour and a common example in the causal-ML literature we are building toward. Each row is a U.S. household from the 1991 Survey of Income and Program Participation. The outcome we model is `net_tfa`, the household's **net financial assets** (everything it has saved, net of debt), measured in thousands of dollars. The features are the household's characteristics: income (`inc`, also in thousands), the age and years of education of the reference person, family size, and a set of 0/1 indicators for being married, having two earners, owning a defined-benefit pension (`db`), owning an IRA (`pira`), and owning a home (`hown`). The last column, `e401`, marks whether the household is **eligible** for a 401(k); that is the variable the rest of this stream will treat as a cause, but here it is just one more feature to predict from.
...
That picture is the whole premise of the model we are about to use: to guess a household's savings, find the households most like it and look at what they saved. Before we can do that we need one small piece of preparation. The model compares households by measuring the distance between them, and distance is sensitive to scale: income runs into the hundreds while family size is a single digit, so without a fix the big-numbered feature would dominate every distance. So we put every feature on the same footing, subtracting its mean and dividing by its standard deviation. There is a subtlety we have to respect: those means and standard deviations must be computed on the training data only, never on data we are about to test on, or information leaks across the split we are about to make. The way we enforce this is by bundling the scaling step together with the model into a single object, a **pipeline**, so that every time we fit, the scaling is recomputed on exactly the data being fit and nothing else.
...
The preview above shows `X` in its raw units, before any scaling. The standardizing happens *inside* the pipeline at the moment we fit, so we never overwrite the original columns and never have to remember to scale by hand; the readable raw numbers are what we keep in front of us.
...
To see this we need new data. We do not have a second sample of households, so we set one aside: we randomly hold out a chunk of our data, fit the model on the rest, and judge it on the held-out chunk it never saw. This is the **train/test split**.
...
The training MSE is essentially zero, and that is the whole point. With $k = 1$ the model predicts each training household by looking for its single nearest neighbour, and in the training data every household is its own nearest neighbour, so the model simply hands back the savings figure it already saw. It has memorised the training set. The test MSE is the true number we are looking for, and it is enormous: on households the model was never fit to, copying the single closest match is very inaccurate. 
...
On the training set every prediction lands exactly on the 45-degree line, because the model is reading back values it memorised. On the test set the scatter spreads out wildly, and that spread is the model's skill on households it has never seen. A low error on the data the model was fit to is the model flattering itself; the wide scatter on the right is the truth. The ability to do well on **new** data is called **generalization**, and it is the only thing we actually care about.
...
To predict a household's savings with **k-nearest-neighbours regression**, we do the simplest thing imaginable. We measure how similar that household is to every household in the training data, using all of the features, find the $k$ closest matches, and report the average of their savings. There is no equation to estimate and no coefficients to read off. We just look up the most similar households and average their outcomes.

The number $k$, how many neighbours to average, is something we choose. A value we set ourselves rather than estimate from the data is called a **hyperparameter**. Pick $k$ too small and the model leans on one or two idiosyncratic neighbours; pick $k$ too large and it averages over households that are not really comparable. The rest of this notebook is about how to choose $k$. This is also why we scaled the features back in section 2: KNN decides who counts as a neighbour by measuring distance, so a feature measured in big numbers would otherwise dominate that distance.
...
Savings rise toward the top-right, where households are older and higher-income, exactly the corner where well-off near-retirees sit and further away from younger adults with lower income. This is exactly what we expect to see. It is important to note that KNN never wrote down that rule; it simply averaged neighbours.
...
At $k = 1$ the model is so flexible that its predictions swing all over the place from one sample to the next: variance is high, bias is almost nothing. As $k$ grows the predictions steady down (variance falls) but the model starts averaging across genuinely different points, so it can no longer follow the curve and a systematic gap opens up (bias rises). The black line is U-shaped, and its bottom is the $k$ that predicts best. This visualization describes how we choose our hyperparameters like $k$ and we will come back to it often in this stream. 
...
The tuned model beats the predict-the-average baseline by a clear margin. It is not a spectacular predictor, because a household's savings depend on a great deal we did not measure, but it is an improvement, and throughout this stream we will learn more techniques to improve our model and its predictive power. 
...
## 8. So have we answered the savings question?
...
**No.** This is one of the core concepts of this stream. Here are the same three reasons stated in general terms, the reasons Notebook 4 returns to:

1. **Prediction can lean on correlates, not causes.** Our model can predict savings well by leaning on features that merely travel *alongside* eligibility: income, age, owning an IRA. Eligibility itself might be doing little of the work the comparison credits it with, and a predictive model cannot tell the difference.
2. **A predictive number is not a causal one.** Even if we read a number for the effect of eligibility out of a model that predicts savings well, that number blends the true effect with **selection**: eligible households already differ from ineligible ones in many other ways, tending to be richer, older, and already inclined to save.
3. **A prediction model cannot imagine anything any differently.** A causal question is a *counterfactual* one: what would happen to *this household's* savings if it became eligible, holding everything else fixed? A prediction model never answers that. The best it can do is point to a *different* household that already is eligible. Those two things, the same household under a change versus a different household that already had it, are not the same, and telling them apart is the heart of what makes causal estimation hard. It is what Notebook 4 begins to fix.

Prediction is one job. Causal estimation is a different job. The next two notebooks sharpen the prediction toolkit; from Notebook 4 onward we turn that toolkit into something that can actually answer the eligibility question.
...
- **Forward to Notebook 4:** the gap between a model that predicts savings well and the actual effect of eligibility, the subject of section 8, is the problem the causal half of this stream exists to solve.
...
- Poterba, J. M., Venti, S. F., & Wise, D. A. (1995). *Do 401(k) contributions crowd out other personal saving?* Journal of Public Economics, 58(1), 1-32. The original study behind the eligibility question.
