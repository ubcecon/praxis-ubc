...
OLS is far worse than predicting the mean. Ridge and lasso are both more than ten times better than OLS and clearly beat the baseline, with very similar test error, lasso doing it with 14 variables and ridge with all 61. You have run the same recipe as Notebook 1, with $\lambda$ in the role $k$ played there.
