...
date: 2026-06-19
image: /media/stream_icons/causal-ml-2.png
...
So we have 90 countries and 61 covariates. 61 coefficients to fit 90 numbers leaves almost no room to spare, and once we split off a test set it gets far worse.
...
- **Back:** [Notebook 1](notebook_01_prediction_inference_causality.qmd) of this stream. There a single hyperparameter $k$ traded bias against variance; here the penalty $\lambda$ does the same, and the number of covariates is the flexibility which causes issues.
- **Forward to [Notebook 3](notebook_03_trees_forests_boosting.qmd):** we stayed with linear models here. Next we go to nonparametric models with **decision trees, random forests, and boosting**, a different way to be flexible.
