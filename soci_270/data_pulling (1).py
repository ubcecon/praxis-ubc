...
def sample_df(df, n=30000, random_state=42):
    """Return a random sample of n rows from df (or all rows if less than n)."""
    if len(df) > n:
        return df.sample(n=n, random_state=random_state)
    else:
        return df
print("Loaded War Propaganda")
print("Loaded Sentiment140")
print("Loaded RuSentitweet")
...
def create_disinformation_dataset():
    ...
    # this samples 30,000 rows from each df
    df_ira = sample_df(df_ira)
    df_war_propaganda = sample_df(df_war_propaganda)
    df_control_en = sample_df(df_control_en)
    df_control_ru = sample_df(df_control_ru)
