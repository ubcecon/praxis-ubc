...
# Prerequisites
SOCI 415 Network Analysis Notebook
...
The countries in the dataset are the United Kingdom, Germany, Poland, Italy, Sweden, Denmark, Finland, Norway, the Netherlands and the USA. 

## Downloading the Data

Students will have to download the data for legal reasons and then place them into the notebook's working directory. 

First navigate to [The KINMATRIX Data page](https://kinmatrix.eu/data/) from there click on the **large red Data download button**. You will need to make an account on GESIS. Once you have created the account on the right-hand side there is a panel with different download options. Press **Datasets** and select **ZA8825_v1-0-0.dta (Dataset) 112.56 MB**, make sure it is the .dta version. 

This will start a download, once it is completed copy the file and paste it into the datasets folder in the same directory as the notebook. There will already be one file there called latest.db make sure it is in the same folder.
...
file_path = r'datasets/ZA8825_v1-0-0.dta' #path
...
The results are interesting we can see that the smallest family networks are in Germany (16.33 nodes per anchor) and the largest are in the United States (22.72 nodes per anchor). We can also see that there are more women than men in most countries especially in the US (2,992 women compared to 1,956 men). 
...
![Location of Pomerania](media/pomorskie.png) 
...
The way in which we will define healthy is if the respondent says his health is either "Very Good" or "Good." As for education we will define someone who is educated if they answered the "High Response."
...
We can see that the finding may be surprising for most people. The United States being the most educated and Germany being the least does seem strange at first glance, but these findings are correct according to the data.

The reason for these findings are the sampling methodology used by the Researchers. In the [Methods Report](https://www.demographic-research.org/volumes/vol51/25/51-25.pdf), we can find that the KINMATRIX researchers explicitly acknowledge that their data shows "Substantial under-representation of lower-educated people in all countries" and "No accurate representation of country differences across intermediate and higher education levels" (8). This is inflating the higher education percentages across the countries in our dataset. Because of the nature of education this bias towards higher education also "extends also to their parental background" (14) and further. 

Another reason for this is bias is the manner in which the data was collected. The key reason is that the researchers did not do random sampling, but rather quota sampling from Dynata access panels which hosted an online survey. This method may lead to the exclusion of people without the internet and a computer as there was no mobile access. This is a known limitation of web-based, non-probability sampling that affects social surveys and the researchers are aware of it.  
...
The way in which KINMATRIX categorizes US states is in regions. The regions are New England, South Atlantic, Mid-Atlantic, East South Central, East North Central, Pacific, West South Central, Mountain, and West North Central. Based on these regions and the Cook Partisan Voting Index (PVI). 
...
Let's see if this tracks across countries and is not just a phenomenon present in the United States. Poland politically is a similarly divided nation with the more Eastern and Southern regions preferring conservative candidates and the Western and Northern regions preferring more liberal candidates. Below is displayed a Map of the 2023 Polish Parliamentary elections. 

![Map of the 2023 Polish Parliamentary elections (For Sejm and Senate)](media/2023_poland.jpg) 

The Polish presidential Election had its second round on June 1, 2025 and this is the map of the election. 

![Map of this Presidential Election](media/2025_poland.webp) 

This general trend of political allegiance has been around since Poland became independent from the USSR in 1989. Following the analysis in the United States using the Polish election results over the last 20 years the four most left-leaning voivodeships are: Mazowieckie (Warsaw), Pomorskie (Gdansk/Tri-city), Wielkopolskie (Poznan) and Zachodniopomorskie (Szczeczin). The four most right-leaning voivodeships are: Podkarpackie (Rzeszow/Przemysl) Lubelskie (Lublin), Podlaskie (Bialystok), and Świętokrzyskie (Kielce). We will repeat the analysis for the United States with these eight voivodeships.
...
Just like with the United States the trend of right-leaning Voivodeships having larger families is true. The average kin per anchor in left-leaning regions is 18.54 and in right-leaning regions is 20.85 a difference of 2.31 which is larger than the 1.58 in the United States. 
...
### 6.3 Significance of these findings 
...
For the next part of the notebook turn to a person next to you and discuss these findings. Why do these more conservative regions have larger families is it just more "traditional values," or is there something more? Use the frameworks discussed in class. This will be easier for the United States as people are more familiar with politics and life in the United States. But also try for Poland; this will be more difficult. 
...
summary_vars = {
    ...
    'religion1': 'anc_rel1',                  # Religion (dummy for religious)
...
![Map of Employment in Poland and Polish cities](media/registered_unemployed_persons_and_unemployment_rate-02.png) 
...
# 8.0 Parental Separations 

Let's continue our analysis by looking at the Parental Separation Variable and seeing if we can infer any trends or interesting findings like before. 
...
It is a simple indicator variable of if the anchor's parents are separated.

We will look through all countries in our dataset looking at countries and parental separation for the parents of the anchor individual. 
```{python}
# Country mapping dictionary
country_map = {
    'UK': '1. UK',
...
Surprising results with Poland and Italy at the top, we would not expect more traditional and Christian countries to have such a high divorce rate. Let's re-use the right and left leaning regions in Poland and see if parental separation differs by political allegiance. 
...
We can see that there is no difference between left and right leaning regions with the Left-leaning regions having a separation percentage of 66.69% and the right leaning regions having one of 69.57%. So the cause is not political leaning. Let us also repeat this analysis for the United States.   
...
* `anc_rel2` is a qualitative variable for which religion the person believes in.
* `anc_rel3` is a variable for the frequency or attendance (how devout the person is).
...
As we expected Republican regions are more religious with 53% of the respondents report that they are religious, compared to 43% in Democrat regions. Respondents from Republican regions also attend religious services more often (14.7% vs 11.7%). 

What is surprising is that it seems that religious families have a significant higher percentage of parental separation for both Democrat and Republican leaning regions. 
...
* Poland has no difference in parental separation based on political regions, but the United States does. Why is that?
...
For the last part of this notebook let's look at which countries have the most straight and non-straight respondents. To do this we will use the **anc_sor** variable. As with all variables let's look at all the values this variable can take. 
```{python}
...
We will look at the regions within each country that have the most heterosexual and non-heterosexual respondents. 
...
Opole is the smallest and least populated voivodeship (province) of Poland. The province's name derives from that of the region's capital and largest city, Opole. Students should already be familiar with Pomerania (Pomorskie) as we have already used and discussed this voivodeship in this notebook. Pomorskie is in the north of Poland on the Baltic sea with the largest city being Gdansk which is a large port along with Gdynia in the region. 

Let's now look at the findings for Denmark:
```{python}
...
![Syddanmark](media/1280px-Syddanmark_in_Denmark.png) 
...
![Nordjylland](media/Nordjylland_in_Denmark.png) 
...
Alternatively students can plot their own family networks on our [Custom Ego-Network Build Website](https://alexr951.github.io/EgoNetworkApp/). 
...
Graves, Jonathan, et al. “Network Analysis for Anthropology.” COMET, UBC, 2024, comet.arts.ubc.ca. 
